...
def generate_report(history):
    ...
    with open(REPORT_FILE, 'w') as f:
        ...
        f.write(f"- Files With Issues: {current['files_with_issues']} ({current['files_with_issues']/current['total_files']*100:.1f}%)\n")
        f.write(f"- Total Issues: {current['total_issues']}\n")
        f.write(f"- Average Issues Per File: {current['total_issues']/current['total_files']:.2f}\n\n")
        ...
        if previous:
            ...
            f.write(f"- Issues Fixed: {issue_diff if issue_diff >= 0 else '⚠️ ' + str(abs(issue_diff)) + ' new issues'}\n")
            f.write(f"- Files Improved: {file_diff if file_diff >= 0 else '⚠️ ' + str(abs(file_diff)) + ' more files have issues'}\n\n")
        ...
        for issue_type, count in current['issues_by_type'].items():
            if count > 0:
                percentage = count / current['total_issues'] * 100 if current['total_issues'] > 0 else 0
                f.write(f"| {issue_type.capitalize()} | {count} | {percentage:.1f}% |\n")
        f.write("\n")
        ...
        sorted_files = sorted(current['issues_by_file'].items(), key=lambda x: x[1], reverse=True)
        ...
        if len(history) > 1:
            ...
            for record in history[-5:]:
                date = datetime.datetime.fromisoformat(record['timestamp']).strftime('%Y-%m-%d')
                f.write(f"| {date} | {record['total_issues']} | {record['files_with_issues']} |\n")
        ...
        if issue_types['docstring'] > 0:
            f.write("- **Add missing docstrings**: Documentation is key to maintainable code\n")
        if issue_types['naming'] > 0:
            f.write("- **Fix naming convention issues**: Follow the naming conventions guide\n")
...
def main():
    ...
    if len(history) > 1:
        ...
        if issue_diff > 0:
            print(f"{GREEN}You've fixed {issue_diff} issues since the last report!{RESET}")
        elif issue_diff < 0:
            print(f"{YELLOW}There are {abs(issue_diff)} new issues since the last report.{RESET}")
        else:
