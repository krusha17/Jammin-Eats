...
class LintingError:
    ...
    def __str__(self):
        return (
            f"{self.filename}:{self.line}:{self.col}: {self.error_type}: {self.message}"
        )
...
class BasicLinter(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        if not FUNCTION_NAME_PATTERN.match(node.name) and not node.name.startswith(
            "__"
        ):
            self.errors.append(
                LintingError(
    ...
    def visit_Assign(self, node):
        ...
        for target in node.targets:
            if (
                ...
                and not CONSTANT_NAME_PATTERN.match(target.id)
            ):
                self.errors.append(
                    LintingError(
                        self.filename,
...
def lint_file(filepath):
    ...
    if not content.lstrip().startswith('"""') and not content.lstrip().startswith(
        ...
        errors.append(
            LintingError(filepath, 1, 0, "Missing module docstring", "docstring")
        )
    ...
    try:
        ...
        errors.append(
            LintingError(filepath, e.lineno, e.offset, f"Syntax error: {e}", "syntax")
        )
