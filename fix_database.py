    ...
    try:
        ...
        if "id" not in [col.lower() for col in columns]:
            ...
            try:
                ...
                cursor.execute("CREATE TABLE player_profile_temp (id INTEGER PRIMARY KEY, name TEXT NOT NULL, tutorial_complete INTEGER DEFAULT 0)")
                ...
                if "name" in columns and "tutorial_complete" in columns:
                    ...
                    for i, record in enumerate(existing_data):
                        name = record[name_index] if len(record) > name_index else "Player"
                        tutorial = record[tutorial_index] if len(record) > tutorial_index else 0
                        cursor.execute("INSERT INTO player_profile_temp (id, name, tutorial_complete) VALUES (?, ?, ?)", 
                                      (i+1, name, tutorial))
                
                # Drop the old table and rename the new one
                print("- Replacing old table with new one...")
                cursor.execute("DROP TABLE player_profile")
                cursor.execute("ALTER TABLE player_profile_temp RENAME TO player_profile")
                conn.commit()
        ...
        for player in players:
            ...
            print(f"- Player ID: {player_id}, Tutorial completed: {bool(tutorial_status)}")
