...
# Create buttons
start_button = Button(
    WIDTH // 2 - 100, HEIGHT // 2, 200, 50, "Start", GREEN, (100, 255, 100)
)
exit_button = Button(
    WIDTH // 2 - 100, HEIGHT // 2 + 70, 200, 50, "Exit", RED, (255, 100, 100)
)
        ...
        if game_state == MENU:
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if start_button.is_clicked(event):
                    ...
                    print(
                        f"[DEBUG] Changing game state from {game_state} to PLAYING ({PLAYING})"
                    )
                    game_state = PLAYING
