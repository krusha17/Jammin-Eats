        ...
        try:
            ...
            if hasattr(self.game, "current_state") and self.game.current_state:
                ...
                try:
                    ...
                    text = font.render(
                        "No active game state to display", True, (255, 0, 0)
                    )
                    screen.blit(
                        text,
                        (
                    ...
                    game_logger.error(
                        f"Could not draw error message: {font_error}", "GameRenderer"
                    )
        except Exception as e:
            game_logger.error(
                f"Error in draw_current_state: {e}", "GameRenderer", exc_info=True
            )
