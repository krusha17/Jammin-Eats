...
class Game:
    ...
    def __init__(self, screen_width=WIDTH, screen_height=HEIGHT):  # noqa: PLR0915
        ...
        try:
            ...
            game_logger.critical(
                f"Failed to initialize pygame display: {e}", "Game", exc_info=True
            )
        ...
        try:
            ...
            db_init = __import__(
                db_init_module,
                fromlist=["initialize_database", "check_database_integrity"],
            )
            ...
            if db_init_success:
                ...
                if not integrity_result:
                    game_logger.warning(
                        f"Database integrity issues detected: {missing_tables}", "Game"
                    )
            else:
            ...
            game_logger.error(
                f"Failed to import database modules: {ie}", "Game", exc_info=True
            )
            self.persistence = None  # Set to None as fallback
        except Exception as e:
            game_logger.error(
                f"Failed to initialize persistence: {e}", "Game", exc_info=True
            )
            self.persistence = None  # Set to None as fallback
        ...
        try:
            ...
            game_logger.critical(
                f"Failed to create sprite groups: {e}", "Game", exc_info=True
            )
        ...
        try:
            ...
            if self.persistence is None:
                game_logger.warning(
                    "Cannot check tutorial status: persistence unavailable", "Game"
                )
                ...
                if not tutorial_completed:
                    ...
                    game_logger.info(
                        "Tutorial not completed, enabling tutorial mode", "Game"
                    )
                else:
                    game_logger.info("Tutorial already completed", "Game")
        except Exception as e:
            game_logger.error(
                f"Error checking tutorial completion: {e}", "Game", exc_info=True
            )
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                try:
                    ...
                    game_logger.error(
                        f"Error in state event handling: {e}", "Game", exc_info=True
                    )
            ...
            if self.game_state == PLAYING and not isinstance(
                ...
                try:
                    ...
                    game_logger.debug(
                        f"Exiting current state: {type(current_state).__name__}", "Game"
                    )
                    current_state.exit()
                    ...
                    if use_simplified:
                        ...
                        try:
                            ...
                            black_screen_module = (
                                f"{IMPORT_PREFIX}states.black_screen_gameplay_state"
                            )
                            black_screen_module = __import__(
                                black_screen_module,
                                fromlist=["BlackScreenGameplayState"],
                            )
                            game_logger.debug(
                                "Creating new BlackScreenGameplayState instance", "Game"
                            )
                            current_state = (
                                black_screen_module.BlackScreenGameplayState(self)
                            )
                            game_logger.info(
                                "Successfully created BlackScreenGameplayState", "Game"
                            )
                        except Exception as e:
                            game_logger.error(
                            ...
                            game_logger.warning(
                                "Falling back to regular GameplayState", "Game"
                            )
                    ...
                    game_logger.debug(
                        f"Entering {type(current_state).__name__}", "Game"
                    )
                    current_state.enter()
                    game_logger.info(
                        f"Successfully transitioned to {type(current_state).__name__}",
                        "Game",
                    )
                    ...
                    try:
                        ...
                        game_logger.critical(
                            f"Could not recover from transition error: {recover_error}",
                            "Game",
                            exc_info=True,
                        )
                    continue
            ...
            if (
                hasattr(current_state, "next_state")
                and current_state.next_state is not None
            ):
                game_logger.info(
                ...
                try:
                    ...
                    game_logger.critical(
                        f"Error during state transition: {e}", "Game", exc_info=True
                    )
                    # Try to recover
                    try:
                        current_state = TitleState(self)
    ...
    def run_gameplay(self):
        ...
        """
        game_logger.warning(
            "run_gameplay() called directly - this method is deprecated", "Game"
        )
    ...
    def draw_current_state(self, screen):
        ...
        try:
            ...
            if (
                ...
                and hasattr(self.shop, "visible")
                and self.shop.visible
            ):
                self.shop.draw(screen)
                game_logger.debug("Drew shop UI", "Game")

            game_logger.debug(
                "Completed draw_current_state method successfully", "Game"
            )

        except Exception as e:
            game_logger.error(
                f"Error in draw_current_state: {e}", "Game", exc_info=True
            )
...
def render(self):
    ...
    if self.selected_food:
        selected_text = font.render(
            f"Selected: {self.selected_food}", True, (255, 255, 255)
        )
