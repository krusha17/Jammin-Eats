...
try:
    ...
    from core.constants import CUSTOMER_SPAWN_RATE
except ImportError:
    # Fall back to src-prefixed imports
    from src.debug.logger import game_logger
    from src.core.constants import CUSTOMER_SPAWN_RATE
...
class GameWorld:
    ...
    def __init__(self, game):
        ...
        try:
            ...
            try:
                ...
                game_logger.error(
                    "Could not import Customer class with any prefix", 
                    "GameWorld"
                )
    ...
    def spawn_customer(self):
        ...
        try:
            ...
            if spawn_points:
                ...
                game_logger.warning(
                    "No valid spawn points found for customer", 
                    "GameWorld"
                )
        except ImportError as e:
            game_logger.error(
                f"Failed to import Customer class: {e}", 
                "GameWorld"
            )
        except Exception as e:
            game_logger.error(
                f"Error spawning customer: {e}", 
                "GameWorld", 
                exc_info=True
            )
    ...
    def setup_tutorial_objects(self):
        ...
        try:
            ...
            if not hasattr(self.game, 'player') or self.game.player is None:
                ...
                game_logger.warning(
                    "Created minimal player fallback for tutorial", 
                    "GameWorld"
                )
        except Exception as e:
            game_logger.critical(
                f"Unexpected error in tutorial setup: {e}", 
                "GameWorld", 
                exc_info=True
            )
    ...
    def reset_state(self):
        ...
        try:
            ...
            game_logger.error(
                f"Error resetting game state: {e}", 
                "GameWorld", 
                exc_info=True
            )
