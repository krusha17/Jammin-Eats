        ...
        if not self.affordable(upg_id, money):
            ...
            if self.has(upg_id):
                ...
                if missing_reqs:
                    ...
                    log(f"Cannot buy {upg_id}: Missing prerequisites: {', '.join(req_names)}")
                else:
                    cost = UPGRADE_DATA[upg_id]["cost"]
                    log(f"Cannot buy {upg_id}: Not enough money (costs ${cost}, have ${money})")
            return 0
    ...
    def apply_upgrades(self, game):
        ...
        for upg_id in self.owned:
            ...
            if "food_lifespan" in mods:
                ...
                log(f"Applied food lifespan bonus: +{mods['food_lifespan']}s from {upg_id}")
                
            # Apply patience multiplier (stacks multiplicatively)
            if "patience_mul" in mods:
                patience_mul *= mods["patience_mul"]
                log(f"Applied patience multiplier: {mods['patience_mul']}x from {upg_id}")
