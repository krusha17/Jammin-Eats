...
def migrate_database(conn):
    ...
    if "money" not in columns:
        ...
        try:
            cursor.execute(
                "ALTER TABLE player_profile ADD COLUMN money INTEGER DEFAULT 0"
            )
            conn.commit()
    ...
    if "successful_deliveries" not in columns:
        ...
        try:
            cursor.execute(
                "ALTER TABLE player_profile ADD COLUMN successful_deliveries INTEGER DEFAULT 0"
            )
            conn.commit()
            log(
                "Migration successful: Added 'successful_deliveries' column to player_profile"
            )
        except sqlite3.Error as e:
            log_error(
                f"Migration failed: Could not add 'successful_deliveries' column: {e}"
            )
            conn.rollback()
    ...
    try:
        ...
        for create_statement in CREATE_TABLES:
            try:
                ...
                table_name = (
                    create_statement.split("EXISTS")[1].split("(")[0].strip()
                    if "EXISTS" in create_statement
                    else "unknown table"
                )
        ...
        if cursor.fetchone():
            ...
            if count == 0:
                ...
                cursor.execute(
                    "INSERT INTO player_profile (id, name, high_score, tutorial_complete) VALUES (1, 'Player', 0, 0)"
                )
                conn.commit()
