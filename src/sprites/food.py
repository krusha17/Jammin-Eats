...
import os
import math
from src.core.constants import ASSETS_DIR, WIDTH, HEIGHT
...
class Food(pygame.sprite.Sprite):
    def __init__(self, x, y, dx, dy, food_type="pizza"):
        ...
        try:
            ...
            preferred_path = os.path.join(
                ASSETS_DIR, "Food", base_name, f"{file_prefix}{cycle_num}.png"
            )
                ...
                if os.path.isdir(food_dir):
                    ...
                    if not self.image and file_prefix != base_name:
                        ...
                        if os.path.exists(alt_path):
                            self.image = pygame.image.load(alt_path).convert_alpha()
                            print(
                                f"Loaded with standard name: {base_name}{cycle_num}.png"
                            )
            ...
            if food_type == "pizza":
                ...
                pygame.draw.polygon(
                    self.image, (200, 0, 0), [(8, 8), (24, 8), (16, 24)]
                )
                ...
                pygame.draw.polygon(
                    self.image, (240, 220, 180), [(8, 28), (24, 28), (16, 10)]
                )
        ...
        self.collision_radius = (
            12  # Smaller than the sprite's visual size for tighter collisions
        )
        ...
        if frame_count > 1:
            ...
            if time_left <= self.hold_last_frame_time:
                ...
                if self.frame_timer >= self.animation_speed:
                    ...
                    ) % frame_count  # loop through all frames
                target_index = self.current_frame
            if (
                target_index != self.current_frame
                or self.image != self.frames[target_index]
            ):
                self.current_frame = target_index
                current_center = (
                    self.rect.center
                )  # preserve position when image size differs
    ...
    def collides_with(self, other_sprite):
        ...
        other_radius = getattr(
            other_sprite, "collision_radius", 20
        )  # Default to 20px if not defined
