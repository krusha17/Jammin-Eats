...
class Customer(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        pygame.draw.polygon(
            self.bubble, (255, 255, 255), [(10, 50), (30, 50), (20, 60)]
        )
        ...
        try:
            ...
            self.food_image = load_image(
                f"food/{base_name}", f"{base_name}1.png"
            ) or load_image("food", f"{base_name}1.png")
            if self.food_image:
                ...
                print(
                    f"[DEBUG] Icon missing, using fallback for {self.food_preference}"
                )
    ...
    def feed(self, food_type):
        ...
        if food_type == self.food_preference:
            ...
            self.satisfaction_timer = (
                0  # Will count up to show satisfaction before leaving
            )
            self.satisfaction_delay = (
                1.0  # 1 second of visible happiness before leaving
            )
        else:
            # Wrong food, get angry but don't leave yet
            print(
                f"[DEBUG] Customer {self.type} received wrong food! Wanted {self.food_preference}, got {food_type}"
            )
            self.state = "angry"
            self.image = self.sprites[self.state]
    ...
    def _draw_fallback_food_icon(self):
        ...
        if self.food_preference == "pizza":
            pygame.draw.polygon(
                self.bubble, (255, 200, 0), [(40, 10), (60, 30), (20, 30)]
            )
            ...
            pygame.draw.polygon(
                self.bubble, (240, 220, 180), [(30, 35), (50, 35), (40, 15)]
            )
    ...
    def update(self, dt):
        ...
        if self.fed and not self.leaving and hasattr(self, "satisfaction_timer"):
            ...
            if self.satisfaction_timer >= self.satisfaction_delay:
                print(
                    f"[DEBUG] Customer {self.type} is now leaving after showing satisfaction"
                )
                self.leaving = True
        ...
        if not self.leaving or self.leave_timer < 1.0:  # Only draw if still visible
            ...
            if not self.fed and not self.leaving:
                ...
                if patience_ratio < 0.7:
                    ...
                    pulse_value = (
                        (pygame.time.get_ticks() * flash_speed) % 1000 / 1000.0
                    )
