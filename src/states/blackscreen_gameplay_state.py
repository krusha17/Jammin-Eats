...
class BlackScreenGameplayState(GameState):
    ...
    def __init__(self, game):
        ...
        game_logger.debug("BlackScreenGameplayState initialized", "BlackScreenGameplayState")
        ...
        try:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    game_logger.info("ESC key pressed, returning to title", "BlackScreenGameplayState")
                    ...
                    try:
                        ...
                        game_logger.error(f"Error creating TitleState: {e}", "BlackScreenGameplayState", exc_info=True)
                
                # Toggle debug info with D key
                elif event.key == pygame.K_d:
                    self.show_debug = not self.show_debug
                    game_logger.debug(f"Debug display toggled: {self.show_debug}", "BlackScreenGameplayState")
                    return True
        
        except Exception as e:
            game_logger.error(f"Error handling event: {e}", "BlackScreenGameplayState", exc_info=True)
    ...
    def update(self, dt):
        ...
        try:
            ...
            game_logger.error(f"Error in update: {e}", "BlackScreenGameplayState", exc_info=True)
    ...
    def draw(self, screen):
        ...
        try:
            ...
            # Instructions
            instructions = self.small_font.render("Press ESC to return to title screen", True, WHITE)
            screen.blit(instructions, (WIDTH // 2 - instructions.get_width() // 2, HEIGHT // 2))
            
            # Show elapsed time
            time_text = self.small_font.render(f"Time in state: {self.elapsed_seconds:.1f}s", True, WHITE)
            ...
            game_logger.error(f"Error drawing: {e}", "BlackScreenGameplayState", exc_info=True)
    ...
    def enter(self):
        ...
        game_logger.info("Entering BlackScreenGameplayState", "BlackScreenGameplayState")
    ...
    def exit(self):
        ...
        game_logger.debug(f"Total time in state: {self.elapsed_seconds:.1f}s", "BlackScreenGameplayState")
