...
class GameplayState(GameState):
        ...
        try:
            ...
            if event.type == pygame.KEYDOWN and not (self.shop and self.shop.visible):
                ...
                if event.key in food_keys:
                    ...
                    game_logger.debug(
                        f"Selected food: {selected_food}", "GameplayState"
                    )
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_b and self.shop:
                    ...
                    game_logger.info(
                        f"Shop {'opened' if self.shop.visible else 'closed'}",
                        "GameplayState",
                    )
                    return True
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    try:
                        ...
                        game_logger.debug(
                            "Transitioning to TitleState", "GameplayState"
                        )
                    except ImportError as e:
                        game_logger.error(
                            f"Failed to import TitleState: {e}", "GameplayState"
                        )
            ...
            game_logger.error(
                f"Error in gameplay event handling: {e}", "GameplayState", exc_info=True
            )
            return False
    ...
    def update(self, dt):
        ...
        try:
                ...
                if self.player and hasattr(self.player, "selected_food_type"):
                    try:
                        ...
                        if hasattr(self.hud, "update"):
                            ...
                            game_logger.warning(
                                "HUD missing update method, using set_selection as fallback",
                                "GameplayState",
                            )
            ...
            if self.player:
                ...
                if (
                    hasattr(self.player, "selected_food_type")
                    and hasattr(self.game, "selected_food")
                    and self.player.selected_food_type != self.game.selected_food
                ):
            ...
            game_logger.error(
                f"Error in GameplayState.update: {e}", "GameplayState", exc_info=True
            )
        ...
        try:
            if hasattr(self.game, "purchased_upgrades"):
                for purchased_upgrade in self.game.purchased_upgrades:
                    if (
                        "speed" in purchased_upgrade
                        and self.player
                        and hasattr(self.player, "speed")
                    ):
                        self.player.speed += 20  # Increase player speed
                        game_logger.debug(
                            f"Player speed increased to {self.player.speed}",
                            "GameplayState",
                        )
                    elif (
                        "inventory" in purchased_upgrade
                        and self.player
                        and hasattr(self.player, "food_inventory")
                    ):
                        self.player.food_inventory += 10  # Increase inventory
                        game_logger.debug(
                            f"Player inventory increased to {self.player.food_inventory}",
                            "GameplayState",
                        )
    ...
    def draw(self, screen):
        ...
        try:
            ...
            game_logger.error(
                f"Error in GameplayState.draw: {e}", "GameplayState", exc_info=True
            )
    ...
    def enter(self):
        ...
        if not self.player or not self.game_map:
            game_logger.warning(
                "Missing required game objects, attempting to initialize",
                "GameplayState",
            )
        ...
        if not self.shop and hasattr(self.game, "initialize_shop"):
            game_logger.info(
                "Creating shop instance during state enter", "GameplayState"
            )
        ...
        if self.player:
            if (
                not hasattr(self.player, "selected_food_type")
                or not self.player.selected_food_type
            ):
    ...
    def _check_interactions(self):
        ...
        try:
            ...
            game_logger.error(
                f"Error checking interactions: {e}", "GameplayState", exc_info=True
            )
    ...
    def _draw_ui(self, screen):
        ...
        try:
            ...
            if (
                ...
                and hasattr(self.player, "selected_food_type")
            ):
                selected_text = self.font.render(
                    f"Selected: {self.player.selected_food_type}", True, WHITE
                )
            ...
            if not (self.shop and self.shop.visible):
                ...
                shop_text = self.font.render(
                    "Press B to open shop", True, (200, 200, 200)
                )
                ...
                help_text = self.font.render(
                    "ESC: Menu | SPACE: Throw", True, (200, 200, 200)
                )
