...
class LoadGameState(GameState):
    ...
    def _load_save_slots(self):
        ...
        try:
            ...
            self.save_slots = [
                {"id": -1, "text": "Return to Title", "enabled": True}
            ]
            
            # Set the return option index
            self.return_option_index = 0
            
            game_logger.debug(f"Loaded {len(self.save_slots)} save slots", "LoadGameState")
        except Exception as e:
            game_logger.error(f"Error loading save slots: {e}", "LoadGameState", exc_info=True)
    ...
    def draw(self, screen):
        ...
        try:
            ...
            for i, slot in enumerate(self.save_slots):
                text = slot["text"]
                color = self.selected_color if i == self.current_selected else self.option_color
            ...
            game_logger.error(f"Error drawing LoadGameState: {e}", "LoadGameState", exc_info=True)
        ...
        try:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_UP:
                    # Move selection up
                    self.current_selected = (self.current_selected - 1) % len(self.save_slots)
                    return True
                    ...
                    if self.current_selected == self.return_option_index:
                        ...
                        if slot.get("enabled", True):
                            game_logger.info(f"Selected save slot: {slot['id']}", "LoadGameState")
            ...
            game_logger.error(f"Error handling event in LoadGameState: {e}", "LoadGameState", exc_info=True)
