...
try:
    ...
    from core.constants import WIDTH, WHITE
except ImportError:
    ...
    from src.core.constants import WIDTH, WHITE
...
class TutorialState(GameState):
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key in (
                ...
                if food_idx < len(food_types):
                    ...
                    if hasattr(self.game, "hud"):
                        ...
                        game_logger.debug(
                            f"Selected food changed to {selected}", "TutorialState"
                        )
                    return True
            ...
            # Toggle hints with H key
                ...
                game_logger.debug(
                    f"Tutorial hints {'shown' if self.show_hint else 'hidden'}",
                    "TutorialState",
                )
                return True
            ...
            # Throw food with SPACE
                ...
                game_logger.debug(
                    f"Tutorial food delivered! Total: {self.served_correct}",
                    "TutorialState",
                )
                ...
                if hasattr(self.game, "particles"):
                    try:
                        ...
                        game_logger.error(
                            f"Couldn't create particle effect: {e}", "TutorialState"
                        )
                return True
    ...
    def update(self, dt):
        ...
        game_logger.debug(
            f"Tutorial progress: deliveries={self.served_correct}/{self.target_deliveries}, money=${self.money_earned}/${self.target_money}",
            "TutorialState",
        )
        ...
        if tutorial_complete_delivery and tutorial_complete_money:
            game_logger.info(
                f"All tutorial goals reached! deliveries={self.served_correct}/{self.target_deliveries}, money=${self.money_earned}/${self.target_money}",
                "TutorialState",
            )
            ...
            if not hasattr(self, "next_state") or self.next_state is None:
                try:
                    # Show the tutorial completion overlay
                    game_logger.info(
                        "Transitioning to TutorialCompleteState", "TutorialState"
                    )
                    self.next_state = TutorialCompleteState(self.game)
                except Exception as e:
                    game_logger.error(
                        f"Failed to transition to TutorialCompleteState: {e}",
                        "TutorialState",
                        exc_info=True,
                    )
        else:
            ...
            if not tutorial_complete_delivery and not tutorial_complete_money:
                ...
                remaining_msg = (
                    f"Still need ${self.target_money - self.money_earned} more money"
                )

            game_logger.debug(
                f"Tutorial not yet complete: {remaining_msg}", "TutorialState"
            )
    ...
    def draw(self, screen):
        ...
        delivery_text = progress_font.render(
            f"Deliveries: {self.served_correct}/{self.target_deliveries}", True, WHITE
        )
        money_text = progress_font.render(
            f"Money: ${self.money_earned}/${self.target_money}", True, WHITE
        )
        ...
        if self.show_hint:
            ...
            # Add dismiss hint
            dismiss_text = self.font.render(
                "Press H to toggle hints", True, (200, 200, 0)
            )
    ...
    def enter(self):
        ...
        try:
            ...
            game_logger.debug(
                "Tutorial objects initialized successfully", "TutorialState"
            )
        except Exception as e:
            game_logger.error(
                f"Error setting up tutorial objects: {e}", "TutorialState"
            )

        # Initialize HUD with default food selection if needed
        if hasattr(self.game, "hud"):
            game_logger.debug(
                "Setting HUD default food selection to burger", "TutorialState"
            )
