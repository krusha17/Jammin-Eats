...
class OptionsState(GameState):
    ...
    def _save_settings(self):
        ...
        try:
            ...
            game_logger.error(f"Error saving settings: {e}", "OptionsState", exc_info=True)
    ...
    def draw(self, screen):
        ...
        try:
            ...
            if self.active_category is None:
                ...
                instructions = "↑/↓: Navigate   ←/→: Change Value   Enter: Save   Esc: Back"
            instruction_surf = instruction_font.render(instructions, True, (200, 200, 200))
            instruction_rect = instruction_surf.get_rect(center=(width // 2, height - 40))
            screen.blit(instruction_surf, instruction_rect)
            
        except Exception as e:
            game_logger.error(f"Error drawing OptionsState: {e}", "OptionsState", exc_info=True)
    ...
    def _draw_categories(self, screen, width, height, font):
        ...
        for i, category in enumerate(self.categories):
            text = category["text"]
            color = self.selected_color if i == self.current_category else self.option_color
    ...
    def _draw_category_settings(self, screen, width, height, font):
        ...
        category_name = next((cat["text"] for cat in self.categories if cat["id"] == self.active_category), "Settings")
        ...
        for i, (key, value) in enumerate(category_settings.items()):
            ...
            color = self.selected_color if i == self.active_option_index else self.option_color
    ...
    def handle_event(self, event):
        ...
        try:
            ...
            game_logger.error(f"Error handling event in OptionsState: {e}", "OptionsState", exc_info=True)
    ...
    def _handle_category_navigation(self, event):
        ...
        if event.key == pygame.K_UP:
            ...
            if category["id"] == "back":
                ...
                self.active_option_index = 0
                game_logger.debug(f"Selected category: {category['id']}", "OptionsState")
            return True
    ...
    def _handle_setting_navigation(self, event):
        ...
        if event.key == pygame.K_UP:
            # Move selection up
            self.active_option_index = (self.active_option_index - 1) % len(setting_keys)
            return True
            ...
            if isinstance(current_value, bool):
                ...
                current_idx = difficulties.index(current_value) if current_value in difficulties else 0
                if event.key == pygame.K_LEFT:
            ...
            # Log the change
            game_logger.debug(f"Changed {current_key} to {category_settings[current_key]}", "OptionsState")
            return True
            ...
            self.active_category = None
            game_logger.debug("Returned to category menu without saving", "OptionsState")
            return True
