    ...
    if asset_type == "player" or asset_type.startswith("sprites/characters/kai"):
        ...
        if asset_type.startswith("sprites/"):
            ...
            if asset_name.startswith("down_") or asset_name == "down.png":
                paths = [
                    os.path.join(
                        base_dir, "sprites", "characters", "kai", "kai_down.png"
                    )
                ]
            elif asset_name.startswith("up_") or asset_name == "up.png":
                paths = [
                    os.path.join(base_dir, "sprites", "characters", "kai", "kai_up.png")
                ]
            elif asset_name.startswith("left_") or asset_name == "left.png":
                paths = [
                    os.path.join(
                        base_dir, "sprites", "characters", "kai", "kai_left.png"
                    )
                ]
            elif asset_name.startswith("right_") or asset_name == "right.png":
                paths = [
                    os.path.join(
                        base_dir, "sprites", "characters", "kai", "kai_right.png"
                    )
                ]
            else:
                ...
                paths = [
                    ...
                    os.path.join(
                        base_dir, "sprites", "characters", "kai", f"kai_{asset_name}"
                    ),
                ]
        ...
        if food_type:
            paths.extend(
                [
                    ...
                    os.path.join(
                        base_dir, "food", food_type.lower(), f"{food_type}.png"
                    ),
        ...
        paths = [
            os.path.join(
                ASSETS_DIR, "sprites", "characters", "Customers", customer_filename
            ),
...
def load_image(asset_type, asset_name, fallback_color=(255, 0, 255)):
    ...
    identifier = (
        asset_type.split("/")[-1][0].upper()
        if "/" in asset_type
        else asset_type[0].upper()
    )
