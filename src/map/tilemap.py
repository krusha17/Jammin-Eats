...
from src.core.constants import ASSETS_DIR, WIDTH, HEIGHT, BASE_DIR
    ...
    def load(self, filename, **_):
        ...
        for y in range(0, 32, tile_size):
            for x in range(0, 32, tile_size):
                ...
                pygame.draw.rect(
                    fallback, colors[color_idx], (x, y, tile_size, tile_size)
                )
        return fallback
...
class TiledMap:
    ...
    def _create_fallback_map(self):
        ...
        pygame.draw.rect(
            self.map_surface, (100, 100, 100), (0, 0, self.width, self.height), 5
        )
    ...
    def _extract_tile_collisions(self):
        ...
        for layer in self.tmx_data.visible_layers:
            ...
            if hasattr(layer, "data"):
                ...
                layer_is_unwalkable = (
                    hasattr(layer, "name")
                    and layer.name is not None
                    and layer.name.lower() in unwalkable_layer_names
                )
                ...
                for x, y, gid in layer:
                    ...
                    if (
                        properties and properties.get("collides", False)
                    ) or layer_is_unwalkable:
    ...
    def get_spawn_positions(self, object_name="CustomerSpawn"):
        ...
        for spawn_type, positions in self.spawn_points.items():
            if key_lower in str(spawn_type).lower():
                print(
                    f"[TiledMap] Found spawn type '{spawn_type}' with positions {positions}"
                )
                for pos in positions:
                    if self.is_walkable(pos[0], pos[1]):
                        ...
                        print(
                            f"[TiledMap] Spawn point at ({pos[0]}, {pos[1]}) not walkable"
                        )
                return spawn_points
            ...
            for obj in obj_group:
                ...
                if key_lower in name_lower or key_lower in group_lower:
                    ...
                    if self.is_walkable(pos[0], pos[1]):
                        ...
                        print(
                            f"[TiledMap] Object spawn point at ({pos[0]}, {pos[1]}) not walkable"
                        )
        if spawn_points:
            return spawn_points
