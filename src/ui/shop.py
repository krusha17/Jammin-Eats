import pygame
from src.core.constants import WHITE, BLACK, GREEN, YELLOW, RED, UPGRADE_DATA
...
class ShopButton:
    ...
    def get_status_text(self):
        if self.is_owned():
            ...
            return (
                f"Cost: ${self.cost} (Need ${self.cost - self.game.economy.money} more)"
            )
        else:
...
class ShopOverlay:
    ...
    def visible(self, value):
        ...
        self.close_rect = pygame.Rect(
            self.screen_size[0] - 150, self.screen_size[1] - 70, 100, 40
        )
    ...
    def update_shop_buttons(self):
        ...
        for i, upgrade_id in enumerate(upgrade_ids):
            ...
            button = ShopButton(
                x, y, button_width, button_height, upgrade_id, self.game
            )
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            for button in self.buttons:
                if button.is_clicked(event):
                    ...
                    if (
                        not button.is_owned()
                        and button.is_available()
                        and button.is_affordable()
                    ):
                        ...
                        if success:
                            ...
                            if (
                                hasattr(self.game, "sounds")
                                and "purchase_sound" in self.game.sounds
                                and self.game.sounds["purchase_sound"]
                            ):
    ...
    def draw(self, surface):
        ...
        if hasattr(self.game, "economy"):
            money_text = self.small_font.render(
                f"Available Money: ${self.game.economy.money}", True, GREEN
            )
