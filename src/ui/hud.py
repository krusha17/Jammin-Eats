...
class HUD:
                ...
                for base_dir in base_dirs:
                    for ext in [".png", ".jpg"]:
                        ...
                        if os.path.exists(path):
                            try:
                                self.food_icons[food] = pygame.image.load(
                                    path
                                ).convert_alpha()
                                # Scale to a reasonable size for HUD
                                self.food_icons[food] = pygame.transform.scale(
                                    self.food_icons[food], (32, 32)
                                )
                                found = True
                                game_logger.debug(
                                    f"Loaded food icon: {food} from {path}", "HUD"
                                )
                                break
                            except Exception as e:
                                game_logger.error(
                                    f"Error loading food icon {food}: {e}", "HUD"
                                )
                    if found:
                        break
    ...
    def _draw_food_selection(self, surface):
        ...
        panel_rect = pygame.Rect(
            0, self.height - panel_height, self.width, panel_height
        )
        ...
        for i, (food_type, icon) in enumerate(self.food_icons.items()):
            ...
            text = font.render(
                food_type.capitalize(), True, COLORS.get("WHITE", (255, 255, 255))
            )
            ...
            key_text = key_font.render(
                f"{i+1}", True, COLORS.get("YELLOW", (255, 255, 0))
            )
    ...
    def _draw_selected_indicator(self, surface):
        ...
        try:
            ...
            pygame.draw.rect(
                surface, COLORS.get("YELLOW", (255, 255, 0)), highlight_rect, 2
            )

            # Add a "SELECTED" indicator
            font = pygame.font.SysFont(None, 16)
            sel_text = font.render(
                "SELECTED", True, COLORS.get("YELLOW", (255, 255, 0))
            )
            surface.blit(sel_text, (x - 5, y - 20))
        except ValueError:
            game_logger.warning(
                f"Selected food {self.selected_food} not found in icons", "HUD"
            )

    def _draw_key_hints(self, surface):
        """Draw keyboard shortcut hints"""
        hint_font = pygame.font.SysFont(None, 20)
        hints = ["1-5: Select Food", "SPACE: Throw Food", "B: Shop Menu", "ESC: Exit"]
