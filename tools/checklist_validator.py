    ...
    try:
        ...
        for testcase in root.findall('.//testcase'):
            ...
            if match:
                ...
                passed = len(testcase.findall('./failure')) == 0 and len(testcase.findall('./error')) == 0
        ...
        for line in lines:
            ...
            if match:
                ...
                if test_id in test_results:
                    ...
                    if PASS_MARKER in line or FAIL_MARKER in line or PENDING_MARKER in line:
                        line = re.sub(r'\| (\w+-\d+) \|', f'| {test_id} {status} |', line)
                    else:
        ...
        for i, line in enumerate(updated_lines):
            if '**Status:**' in line:
                ...
                if "Last updated:" in line:
                    updated_lines[i] = re.sub(r'Last updated: .*\*\*', f"Last updated: {timestamp}**", line)
                else:
                    updated_lines[i] = line.replace('**Status:**', f"**Status:** Last updated: {timestamp}**")
                updated = True
                break
...
def update_readme_badge(test_results):
    ...
    try:
        ...
        if test_results:
            ...
            color = "brightgreen" if percentage >= 90 else "yellow" if percentage >= 70 else "red"
