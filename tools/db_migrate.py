...
class MigrationManager:
        ...
        for migration_file in migrations_dir.glob("*.sql"):
            ...
            try:
                ...
                self.migrations.append({
                    "version": version,
                    "path": migration_file,
                    "name": migration_file.stem.split("_", 1)[1] if "_" in migration_file.stem else "unknown"
                })
            except ValueError:
    ...
    def upgrade(self, target="head"):
        ...
        to_apply = [m for m in self.migrations if m["version"] > current and m["version"] <= target]
    ...
    def downgrade(self, steps=1):
        ...
        to_downgrade = [m for m in reversed(self.migrations) 
                        if m["version"] <= current and m["version"] > target]
        
        for migration in to_downgrade:
            try:
                log(f"Downgrading migration {migration['version']}: {migration['name']}")
                with open(migration["path"], "r") as f:
                    sql = f.read()
...
def main():
    ...
    upgrade_parser.add_argument("target", nargs="?", default="head", help="Target version (default: head)")
    
    # Downgrade command
    downgrade_parser = subparsers.add_parser("downgrade", help="Downgrade database schema")
    downgrade_parser.add_argument("steps", type=int, default=1, nargs="?", 
                                help="Steps to downgrade (default: 1)")
    ...
    if args.command == "upgrade":
        ...
        data_dir.mkdir(exist_ok=True)
        migrations_dir.mkdir(exist_ok=True)
        log(f"Initialized migration environment")
